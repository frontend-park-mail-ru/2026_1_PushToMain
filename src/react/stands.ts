	...
	while (index < validElements.length || oldFiber != null) {
		...
		if (sameType) {
			...
			if (oldFiber?.instance) {
				...
				newFiber.instance.__mounted = oldFiber.instance.__mounted === true;
			}
		} else if (element) {
			newFiber = {
				type: element.type,
...
function performUnitOfWork(fiber: Fiber): Fiber | null {
	...
	const isClassComponent =
		isFunctionComponent &&
		(fiber.type as any).prototype?.isDeath13Component === true;
...
function commitRoot() {
	...
	const survivingInstances = mountQueue.filter(
		(instance) => !instance._fiber?.deleted,
	);

	survivingInstances.forEach((instance) => {
...
function commitDeletion(fiber: Fiber, domParent: Node) {
	...
	if (
		...
		if (fiber.instance.__mounted !== false) {
			fiber.instance.componentWillUnmount();
			fiber.instance.__mounted = false;
		}
	}
